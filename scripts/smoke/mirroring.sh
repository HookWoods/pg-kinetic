...
temp_dir_rel="target/smoke/mirroring-$$"
...
[mirror.target]
address = "127.0.0.1:$mirror_target_port"
isolated = true
...
mirror_response="$(wait_for_admin_response "$admin_port" "SHOW MIRRORING;" "off")"
assert_contains "$mirror_response" "sample_rate" "mirror view should show the sample rate"
assert_contains "$mirror_response" "0.000" "mirror view should report the default disabled sample rate"
assert_contains "$mirror_response" "mode" "mirror view should expose the mode column"
