...
Runtime engines plug in at accept and task scheduling boundaries. `thread_per_core` is the default stable engine, while `tokio_default`, `tokio_current_thread`, and Linux `io_uring` remain stable selectable engines. `io_uring` requires a binary compiled with the `io-uring` cargo feature.
