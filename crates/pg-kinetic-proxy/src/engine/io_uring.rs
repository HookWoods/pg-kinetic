...
use crate::{config::Config, observe::metrics};
...
mod linux {
    ...
    use pg_kinetic_core::cluster::runtime::ShutdownReason;

    use crate::{ops::drain::DrainController, ops::lifecycle::wait_for_shutdown_signal};
    ...
    pub fn run(config: Config) -> anyhow::Result<()> {
        ...
        let client_tls_server_config = crate::ops::reload::load_client_tls_server_config(&config)?;
    ...
    async fn run_shard(
        ...
        buffer_pool: crate::net::buffers::ProxyBufferPool,
        client_slots: Arc<tokio::sync::Semaphore>,
        backend_slots: Arc<tokio::sync::Semaphore>,
        drain: crate::ops::lifecycle::DrainToken,
        ...
        let backend_pool_selector = Arc::new(
            crate::engine::io_uring_transport::MonoioBackendPoolSelector::new(
    ...
    async fn proxy_connection(
        ...
        client_tls_server_config: Option<Arc<rustls::ServerConfig>>,
        buffer_pool: crate::net::buffers::ProxyBufferPool,
        backend_pool_selector: Arc<crate::engine::io_uring_transport::MonoioBackendPoolSelector>,
        max_client_buffer_bytes: usize,
        max_backend_buffer_bytes: usize,
    ) -> anyhow::Result<()> {
        let mut client = crate::engine::io_uring_transport::MonoioTransport::new(client);
        let mut client_buffer = BytesMut::with_capacity(16 * 1024);
        let effective_config = runtime_state.effective_config();
        let phase_recorder = crate::observe::telemetry::phase_timing_recorder(false);
        ...
        .await?
        {
            ...
            crate::proxy::StartupOrCancel::Cancel {
                ...
                let mut backend = crate::engine::io_uring_transport::MonoioTransport::new(backend);
                let packet =
                    crate::cancel::encode_cancel_request(target.process_id, target.secret_key);
                crate::engine::io_runtime::write_all_to(&mut backend, &packet)
                    .await
                    .context("forward cancel request")?;
                let _ = crate::engine::io_runtime::shutdown(&mut backend).await;
                return Ok(());
            }
        ...
        let context = crate::proxy::SharedClientSessionContext {
            ...
            auth_users: crate::ops::reload::load_auth_users(effective_config)?,
        ...
        crate::proxy::handle_client_session::<
            _,
            crate::engine::io_uring_transport::MonoioBackend,
            Arc<crate::engine::io_uring_transport::MonoioBackendPool>,
            Arc<crate::engine::io_uring_transport::MonoioBackendPoolSelector>,
