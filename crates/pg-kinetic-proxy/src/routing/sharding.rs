...
use crate::{observe::snapshot::SnapshotStore, ops::reload};
...
fn build_preview_route_map(
    ...
    let routes = route_map
        ...
        .map(|target| {
            ...
            Ok(ShardRoute::new(
                pg_kinetic_core::traffic::sharding::ShardTarget::new(
                    route_key.clone(),
                    backend_role,
                    shard_id,
                ),
...
fn preview_route_health_snapshot(route_maps: &[ShardRouteMap]) -> RouteHealthSnapshot {
    ...
    for (offset, _) in route_maps
        ...
        .filter(|route| {
            route.target().backend_role() == pg_kinetic_core::traffic::routing::BackendRole::Replica
        })
        .enumerate()
    {
        ...
        if let Some(snapshot_store) = snapshot_store {
            if let Some(rebalance_plan) = rebalance_plan {
                snapshot_store.set_shard_migration_safety_snapshot(
                    crate::observe::snapshot::ShardMigrationSafetySnapshot::from(rebalance_plan),
                );
            }
        ...
        ShardExtraction::Key { key, .. } => {
                ...
                select_route_for_shard_key(route_map.routes(), route_map.strategy(), &key)
            {
                let reason = match route_map.strategy() {
                    pg_kinetic_core::traffic::sharding::ShardStrategy::Hash => {
                        ShardRouteReason::HashMatch
                    }
                    pg_kinetic_core::traffic::sharding::ShardStrategy::Range => {
                        ShardRouteReason::RangeMatch
                    }
                    pg_kinetic_core::traffic::sharding::ShardStrategy::List => {
...
fn select_route_for_shard_key<'a>(
    routes: &'a [ShardRoute],
    strategy: pg_kinetic_core::traffic::sharding::ShardStrategy,
    key: &pg_kinetic_core::traffic::sharding::ShardKey,
) -> Option<&'a ShardRoute> {
    if routes.is_empty() {
        return None;
    }
    ...
    match strategy {
        ...
        pg_kinetic_core::traffic::sharding::ShardStrategy::List => match key.as_text() {
            Some(text) => routes
                .iter()
