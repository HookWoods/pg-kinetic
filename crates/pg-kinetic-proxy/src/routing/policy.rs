...
use pg_kinetic_core::{
    ...
    traffic::routing::{BackendRole, QueryClass, RoutingDecision},
    traffic::sharding::ShardRouteDecision,
};

use crate::config::{InlinePolicyActionConfig, InlinePolicyConfig, PolicyConfig};
use crate::observe::snapshot::SnapshotStore;
#[cfg(feature = "policy-wasm")]
...
impl PolicyRuntime {
    ...
    pub fn record_audit_event(
        ...
        crate::observe::metrics::record_policy_audit_event(self.policy_mode, event);
        true
    }
    ...
    pub fn evaluate_wasm_policy(
        ...
        match &result {
            Ok(decision) => crate::observe::metrics::record_policy_wasm_eval(
                "wasm",
                self.policy_mode,
                rule.hook_point,
                decision.outcome,
            ...
            Err(error) => crate::observe::metrics::record_policy_wasm_eval(
                "wasm",
                self.policy_mode,
                rule.hook_point,
                error.outcome(),
...
pub fn preview_policy(
    ...
    crate::observe::metrics::record_policy_decision(PolicyMode::DryRun, &audit_event);
...
impl PolicyStore {
        ...
        S::Item: AsRef<str>,
    {
            ...
            next_config.validate_with_context(active_routes, sharding_enabled, active_shards)
        {
            ...
            crate::observe::metrics::record_policy_reload(
                policy_source,
