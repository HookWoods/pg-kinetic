...
fn trace_sampling_ratio_is_clamped_between_zero_and_one() {
    let high = Config::try_parse_from_args([
        "pg-kinetic",
    ...
    let low = Config::try_parse_from_args([
        "pg-kinetic",
        "--debug-trace-sampling-rate=-0.25",
    ])
...
fn span_helpers_exclude_sql_text_and_secret_labels() {
    let forbidden_labels = ["sql", "sql_text", "query", "query_text", "statement", "password", "secret"];
